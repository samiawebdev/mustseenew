...
type Movies = RouterOutputs["tvshow"]["getAll"][0];
type LikesAndPComments = RouterOutputs["likesAndComments"]["getAll"];
...
const Videos = () => {
    ...
    const getData = () => {
        const response = api.tvshow.getAll.useQuery(
            undefined, // no input
            {
        ...
        console.log("tvshows data: ", response.data);
    ...
    //  console.log("tvshows data: ", response);
    ...
    const handleLikeClick = async (e: React.FormEvent<HTMLFormElement>, movie: number) => {
        ...
        if (tvs?.data == undefined || tvs?.data?.length <= 0) {
            return;
        }
        else {
                ...
                <section className=" body-font ">
                    <div className="container px-5 pb-24 pt-8 mx-auto ps-20 cl:ps-0">
                        ...
                        <div className="flex flex-wrap -m-4">
                            ...
                            {tvs?.data ? tvs?.data?.map(function (elt) {
                                return (
                                    ...
                                    <div className="xl:w-1/4 md:w-1/2 p-4" key={elt.id}>
                                        ...
                                        <div className="bg-gray-800 bg-opacity-40 p-6 rounded-lg">
                                            ...
                                            <h3 className="tracking-widest text-indigo-400 text-xs font-medium title-font">SÉRIE</h3>
                                            <h2 className="text-[0.6rem] text-white font-medium title-font mb-4">{elt?.title!.replace('_', ' - ')}</h2>
                                            <div className="grid grid-cols-2 gap-x-4" data-mov={elt.id}>
                                                {/* <button className="backgrop-opacity-95 bg-fuchsia-600 min-h-12 min-w-20 rounded-md">Liker</button>
